...
public class BlueprintAPIController {
    ...
    public ResponseEntity<?> manejadorPostRecursoPlanos(@RequestBody JSONObject blueprint) {
        try {
            ...
            for (String str : list) {
                List<String> listaSeparada = Arrays.asList(str.split(","));
                String posX = listaSeparada.get(0).substring(2);
                String posY = listaSeparada.get(1).substring(2, listaSeparada.get(0).length());
